...
async function TransferGasPriceintoUserwallet(
  symbol,
  gasPrices,
  fee,
  myAddress
...
async function TransferBEP20ERC20(
  withdrawtype,
  amount,
  from_address,
  to_address,
  ...
  if (withdrawtype == 1 || withdrawtype == 2) {
    if (withdrawtype == 1) {
      ...
      ); // mainnet  https://mainnet.infura.io/v3/a1361e1c0fda468bab200985724259bd
    } else {
      var web3 = new Web3API(
        new Web3API.providers.HttpProvider(`https://bsc-dataseed.binance.org/`)
    ...
    return new Promise(async (resolve, reject) => {
      ...
      var encoded_tx = await contract.methods
...
exports.transfer = async function (
  symbol,
  coin_id,
  amount,
  to_address,
  ...
  if (symbol == "BUSD") {
    const response1 = await fetch(
      `http://blockchainexpert.co.in:7003/api/bep20/mainnet/transfer`,
      {
        method: "POST",
        headers: {
  ...
  try {
    if (symbol == "ETH" || symbol == "BNB" || symbol == "MATIC") {
      if (withdrawtype == 0) {
        ...
        const resp = await TransferBEP20ERC20(
          withdrawtype,
          amount,
          from_address,
          to_address,
      ...
      try {
        if (withdrawtype == 0) {
          ...
          // })
          return new Promise(async (resolve, reject) => {
            const privateKey = await openWallet(
              process.env.BTCWalletPrivateKey
            ); //
            ...
            console.log(
              "Address",
              "balanceInSats",
              await getAddressInfo(publickey),
              balanceInSats
            );
            await account
              .sendSats(
      ...
      try {
        return new Promise(async (resolve, reject) => {
          ...
          await axios
            .post(
              "https://api.blockcypher.com/v1/ltc/main/txs/new?token=0e42a92b197845b38e9bd7fdf080ee89",
            ...
            .then(async function (tmptx) {
              ...
              await axios
                .post(
                  "https://api.blockcypher.com/v1/ltc/main/txs/send?token=0e42a92b197845b38e9bd7fdf080ee89",
                  JSON.stringify({
                    tx: tmptx.data.tx,
                    tosign: tmptx.data.tosign,
                    signatures: tmptx.signatures,
      ...
      if (withdrawtype == 0) {
        return new Promise(async (resolve, reject) => {
          try {
            const privateKey = await openWallet(
              process.env.TronWalletPRIVATEKEY
            ); //TESTNET process.env.TronWalletPRIVATEKEY  //42c617f3743b6472bf0768608a09cff63f630fe73d2937dc968550378baf3d8diHrmkLls321jMw3DGBF6VUBWN4RsEjtahbqEVmJ9jSufcrch1piU20O7//eFtP+tF0e8tkhWgc1xZcIgrUQlRoa6IH/S1ybiwMJ9nTHJCAw=
      ...
      if (withdrawtype == 1 || withdrawtype == 2) {
        ...
        return new Promise(async (resolve, reject) => {
          ...
          let Contractaddress =
            withdrawtype == 1 ? contractAddress : Bnb_contract;
          ...
          console.log(
            ...
            parseFloat(checkbalance / 10 ** decimals),
            amount / 10 ** 18
          );
          if (parseFloat(checkbalance / 10 ** decimals) < amount / 10 ** 18) {
            ...
            var encoded_tx = await contract.methods
              .transfer(to_address, amount.toString())
              .encodeABI();
...
async function transferUserToAdminBEPTRC20(
  symbol,
  from_private_key,
  from_address,
  to_address,
  ...
  if (web3 !== "") {
    ...
    console.log(
      "main balance anf tokentransferfee ",
      parseFloat(ETHBalace).toFixed(8),
      (parseFloat(gasPrices) * parseFloat(fixgaslimit)) / 10 ** 18
    );
    if (
      parseFloat(balance / 10 ** decimals) > 0 &&
      parseFloat(ETHBalace) <
        (parseFloat(gasPrices) * parseFloat(fixgaslimit)) / 10 ** 18
    ) {
      await TransferGasPriceintoUserwallet(
        symbol,
        gasPrices,
        (parseFloat(gasPrices) * parseFloat(fixgaslimit)) / 10 ** 18,
        myAddress
...
async function transferUserToAdminTRC20(
  symbol,
  trc_from_private_key,
  trc_from_public_key,
  trcContractAddress
) {
  try {
    ...
    // }
    if (
      parseFloat(balance / 10 ** 6) > 0 &&
      parseFloat(TRCbalance / 10 ** 6) < parseFloat((0.002 * balance) / 10 ** 6)
    ) {
      await TranferGasPriceintoUserWalletTRC20(
        (0.002 * balance) / 10 ** 6,
        fromAddress
      );
    }
...
exports.usertoAdminTransfer = async function (alluserwallets) {
  try {
    ...
    for (let n in alluserwallets) {
      ...
      console.log(
        "alluserwalletsalluserwallets",
        alluserwallets[n].symbol,
        symbol,
        from_private_key
      );
      console.log(
        "alluserwalletsalluserwalletsuserid",
        alluserwallets[n].user_id
      );
      ...
      if (alluserwallets[n].bnb_privatekey !== null) {
        var bnb_from_private_key = await openWallet(
          alluserwallets[n].bnb_privatekey
        );
      ...
      if (symbol == "ETH" || symbol == "BNB" || symbol == "MATIC") {
        ...
        for (let n = 0; n < 2; n++) {
          if (n == 0) {
            ...
            n == 1 &&
            symbol == "ETH" &&
            bnbContractAddress !== null &&
            bnb_from_public_key
          ) {
            await transferUserToAdminBEPTRC20(
              "BNB",
              bnb_from_private_key,
              bnb_from_public_key,
              to_address,
              bnbContractAddress
            );
      ...
      // }
        ...
        for (let x = 0; x < 3; x++) {
          ...
          if (x == 0 && contractAddress && from_address) {
            ...
            await transferUserToAdminTRC20(
              "TRC",
              trc_from_private_key,
              trc_from_public_key,
              trcContractAddress
